import sys
from encryption import encryptFolder
...
def EncryptAction():
	if enc_pass_.get() != "" and enc_pass_.get() == enc_pass_conf.get():
		encryptFolder(filename,enc_pass_.get())
		root.destroy()
...
enc_pass_box_conf.config(show="*")
button_encrypt = ttk.Button(root,text="Encrypt", command=EncryptAction).place(x=300,y=150)
